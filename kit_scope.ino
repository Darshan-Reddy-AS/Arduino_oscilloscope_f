...
// ******** параметры конфигурации вашего полупроводникового тестера
...
// Выберите свой язык:
// Доступные языки: LANG_ENGLISH, LANG_GERMAN, LANG_POLISH, LANG_CZECH, LANG_SLOVAK, LANG_SLOVENE,
//                          LANG_DUTCH, LANG_BRASIL, LANG_RUSSIAN, LANG_UKRAINIAN
#define LANG_ENGLISH

// Опция LCD_CYRILLIC необходима, если у вас дисплей с кириллическим набором символов.
// На этом ЖК-дисплее нет символов для обозначения Ом и u (микро).
// Для русского языка требуется ЖК-контроллер с русским набором символов и опцией LCD_CYRILLIC!
#define LCD_CYRILLIC

// Опция LCD_DOGM должна быть установлена для поддержки ЖК-модулей типа DOG-M с контроллером ST7036.
// Для данного типа ЖК-дисплея контрастность должна быть установлена с помощью программной команды.
//#define LCD_DOGM

// Опция STRIP_GRID_BOARD выбирает другую компоновку платы, не устанавливайте для стандартной платы!
// Подключение ЖК-дисплея полностью отличается для обеих версий.
...
// Опция WITH_AUTO_REF позволяет считывать внутреннее опорное напряжение, чтобы получить коэффициенты для измерения емкости.
#define WITH_AUTO_REF
// REF_C_KORR корректирует опорное напряжение для измерения емкости (<40 мкФ) и имеет единицы измерения в мВ.
// Большие значения дают результаты с меньшей производительностью.
#define REF_C_KORR 12
// REV_L_CORR корректирует опорное напряжение для измерения индуктивности и имеет единицы измерения в мВ.
#define REF_L_KORR 40
// C_H_KORR определяет поправку в 0,1% единиц измерения для измерения большого конденсатора.
// Положительные значения приведут к снижению результатов измерений.
#define C_H_KORR 0
...
// NO_AREF_CAP сообщает вашему программному обеспечению, что у вас не установлен конденсатор на выводе AREF (21).
// Это позволяет сократить время ожидания функции AUTOSCALE_ADC.
// Конденсатор с 1nF можно использовать с установленной опцией NO_AREF_CAP.
//#define NO_AREF_CAP
...
const unsigned char VERSION_str[] MEM2_TEXT = "-- GECR --";
